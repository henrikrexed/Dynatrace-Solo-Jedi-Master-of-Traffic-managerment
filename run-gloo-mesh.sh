...
export DT_ENDPOINT=https://ikq13173.live.dynatrace.com
# export DT_API_TOKEN=
...
cat ./gloo-mesh/gloomesh-values.yaml | sed 's@$DT_ENDPOINT@'"$DT_ENDPOINT"'@' | sed 's@$DT_API_TOKEN@'"$DT_API_TOKEN"'@' > ./gloo-mesh/gloomesh-values-AUTOGENERATED-IGNORE.yaml

meshctl install \
  --kubecontext $CLUSTER_CONTEXT \
  --license $GLOO_MESH_LICENSE_KEY \
  --set global.cluster=$CLUSTER \
  --chart-values-file ./gloo-mesh/gloomesh-values-AUTOGENERATED-IGNORE.yaml
